...
{
    // Vérifiez si l'automate est déterministe
    ...
    // copier l'alphabet dans l'automate minimal
    Automaton minimal;
    minimal.alphabet = deterministic.alphabet;

    // Initialisation de l'ensemble des états finaux et non finaux
    ...
    // Création d'un mappage de l'ancien état vers le nouvel état
    ...
    // Ajouter les nouveaux états à l'automate minimal
    for (int i = 0; i < newState; i++) {
        minimal.addState(i);
    }

    // Définir les états initiaux et finaux
    for (int oldState : deterministic.initialStates) {
        minimal.setStateInitial(stateMapping[oldState]);
    }
    for (int oldState : deterministic.finalStates) {
        minimal.setStateFinal(stateMapping[oldState]);
    }

    // Ajouter les transitions
    for (const auto &transition : deterministic.transitions) {
